...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        % -------------------------------------------------------
        function LoadData(obj, HVVal, PDVal)
            % Use for loading intermediate HV and PD data only.
            obj.HV = HVVal;
            obj.PD = PDVal;
        end
    ...
    methods(Access = private, Static)
        ...
        function Output(obj)
            ...
            if obj.evaluated >= obj.evaluation
                % draw flight route only when running 1 time.
                if obj.run == 1
                    obj.save = 0;
                else
                    obj.save = 1;
                ...
                if obj.save == 0
                    ...
                    % PlotSolution(obj.result{end});

                    % my plot
                    if ~isempty(Feasible)
                    %     PlotSolution(Population);
                        % Calculate HV metric value
                        pop = obj.result{end}(Feasible);
                        objVal = pop.objs;
                        Reference = max(Population.objs, [], 1);
                        HVval = HV(objVal, Reference);
                    %     disp(['HV: ', num2str(HVval)]);

                        PDVal = PD(objVal);
                    %     disp(['PD: ', num2str(PDVal)]);
                    end
                    ...
                    HVval = HV(objVal, Reference);
                    PDVal = PD(objVal);
                    pop_to_save = Population.objs;
                    folder = fullfile('Data',func2str(obj.algorithm));
                    % model = obj.problem.model;
                    [~,~]  = mkdir(folder);
                    % result         = obj.result;
                    % metric.runtime = obj.runtime;
                    save(fullfile(folder,sprintf('%s_%s_M%d_D%d_%d_%f_HV%f_PD%f.mat',func2str(obj.algorithm),class(obj.problem),obj.M,obj.D,obj.run,obj.runtime,HVval,PDVal)),'pop_to_save');
                end
            end
        end
